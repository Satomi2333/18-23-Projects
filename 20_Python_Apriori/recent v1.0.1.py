...
import sys
sys.path.append(r'.\3.x')
import Apriori
# from win32com.shell import shellcon

databasePath = '.\\Data.db'
recentPath = r'C:\Users\a\AppData\Roaming\Microsoft\Windows\Recent\\'
uselessPath = r'.\\'
...
def GetpathFromLink(lnkpath):
    ...
    try:
        ...
        try:
            ...
            print('试图删除出错的目标({})时出错：'.format(lnkpath),end=' ')
            print(e)
        return ''
...
def insertData():
    ...
    print(time.strftime("[%Y-%m-%d %H:%M:%S]:",time.localtime())+"Opened database successfully")
    c = conn.cursor()
    recentList = os.listdir(recentPath)
    for item in recentList:
        ...
        if 'ms-gamingoverlay--startuptips' in name:
            ...
            if tag == 0:
                ...
                try:
                    # print(item)
                    shutil.move(recentPath+item,uselessPath+item)
                    print('移动文件：{}'.format(item))
                except Exception as e:
                    print(e)
            elif tag == 3:
                target = 'unable locate it'
    ...
    conn.commit()
    conn.close()
    print(time.strftime("[%Y-%m-%d %H:%M:%S]:",time.localtime())+"Close database successfully\n")
...
globalMinSupport = 0.5
def doubleFrequent(dataSet,minSupport):
    newData = []
    for data in dataSet:
        frequentItemsets,values,_ = dataProcess(*data,minSupport=globalMinSupport,minConf=1,needRules=False)
        temp = []
...
dataSetRecent = [[(-1,DAYTIME),(-2,DAYTIME),(-3,DAYTIME)],
                 [(-3,DAYTIME),(-4,DAYTIME),(-5,DAYTIME)],
                 [(-2,DAYTIME),(-3,DAYTIME),(-4,DAYTIME)]
                 ]
dataSetNight = [[(-1,NIGHT),(-2,NIGHT),(-3,NIGHT)],
                [(-2,NIGHT),(-3,NIGHT),(-4,NIGHT)]
                ]
dataSetYesterday = [-1,-2,-3]
dataSetWeekago = [0,-7,-14]

#分别为[0]:数据集 [1]:内函数的minSupport [2]:外函数的minSupport [3]:描述 [4]:计算方式
dataSetSet = [(dataSetRecent, 0.6, 0.6,'最近最爱：','doubleFrequent'),
              (dataSetNight, 0.6, 0.4,'最近晚间最爱：','doubleFrequent'),
              (dataSetYesterday, 0, 0.4,'昨天打开过：','singleFrequent'),
              (dataSetWeekago, 0, 0.4,'一周前曾打开过：','singleFrequent')]
for dataSet in dataSetSet:
    globalMinSupport = dataSet[1]
    if dataSet[4] == 'doubleFrequent':
        frequentItemsets,values,rules = doubleFrequent(dataSet[0],dataSet[2])
    elif dataSet[4] == 'singleFrequent':
        frequentItemsets,values,rules = dataProcess(*(dataSet[0]),minSupport=dataSet[2],needRules = False)
    print(dataSet[3])
    showFreItemsets(frequentItemsets,values)
    print()
# showRules(rules,values)

frequentItemsets,values,rules = dataProcess(-1,-2,-3,-4,-5,-6,-7,minSupport=3/7,minConf=0.8)
# showFreItemsets(frequentItemsets,values)
print('打开某个（某些）文件后，有概率会打开的另一个（另一些）文件：')
showRules(rules,values)
