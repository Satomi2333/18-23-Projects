...
x2,y2 = node2.setRandomXY((20,40),(20,40))
x3,y3 = node3.setRandomXY((-45,-30),(-40,-20))
x4,y4 = node4.setRandomXY((10,30),(-30,-10))
...
xEnd = 15
yEnd = 30
x = np.linspace(xStart,xEnd,240)
y = np.linspace(yStart,yEnd,240)
...
#
synMessageStr,xSynMessage,ySynMessage,nodesDoingWhatStr = node1.generateAnimation('gossip',60)
...
def update_points(num):
    ...
    nodesDoingWhat.set_text("node {}:{}     node {}:{}\nnode {}:{}     node {}:{}".format(str(node1),nodesDoingWhatStr[0][num], str(node2),nodesDoingWhatStr[1][num], str(node3),nodesDoingWhatStr[2][num], str(node4),nodesDoingWhatStr[3][num]))
    #nodesDoingWhat.set_text("node {}:{}     node {}:{}\nnode {}:{}     node {}:{}".format(str(node1),node1.doing, str(node2),node2.doing, str(node3),node3.doing, str(node4),node4.doing))
    node1KnownEndPointStates[0].set_text(node1.printKnownEndPointStates(False))
    node1KnownEndPointStates[1].set_text(node2.printKnownEndPointStates(False))
    node1KnownEndPointStates[2].set_text(node3.printKnownEndPointStates(False))
    node1KnownEndPointStates[3].set_text(node4.printKnownEndPointStates(False))
...
ani = animation.FuncAnimation(fig, update_points, 180, interval=10, blit=True)
ani.save('d:\\testOut.gif', writer='imagemagick', fps=30)
#node1.gossipDigestSynMessage(node1.randomOtherNode())
#time.sleep(0.2)
#node3.gossipDigestSynMessage(node3.randomOtherNode())