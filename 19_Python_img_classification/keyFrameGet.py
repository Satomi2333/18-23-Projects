...
def keyframeGet(inputPath = 'D:\\',inputFile = 'test.MP4', outputPath = './extract_result/'):
#    now = time()
    ...
    videopath = inputPath + inputFile
    #Directory to store the processed frames
    dir = outputPath
    #smoothing window size
    len_window = int(50)
    
    fileNamePrefix = inputFile.split('.')[0]
    fileNameSuffix = inputFile.split('.')[-1]
    ...
    idx = 0
    try:
        os.mkdir(outputPath + fileNamePrefix)
    except:
        pass
    while(success):
        if idx in keyframe_id_set:
            name = fileNamePrefix + '\\' + fileNamePrefix + "_keyFrame_" + str(idx) + ".JPG"
            cv2.imwrite(dir + name, frame)
            keyframe_id_set.remove(idx)
        idx = idx + 1
        success, frame = cap.read()
    cap.release()
    #print('used:'+str(time()-now)+'seconds')
